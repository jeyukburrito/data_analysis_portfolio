...
# 1단계: 모델 훈련 쿼리
TRAIN_MODEL_SQL = """
CREATE OR REPLACE MODEL `codeit-final-project.json_to_table.payment_prediction_model_class_weight_{{ get_suffix(ds) }}`
OPTIONS(
...
# 2단계: 모델 성능 평가 쿼리
EVALUATE_MODEL_SQL = """
CREATE OR REPLACE TABLE `codeit-final-project.json_to_table.payment_evaluation_results_class_weight_{{ get_suffix(ds) }}` AS
SELECT *
FROM ML.EVALUATE(MODEL `codeit-final-project.json_to_table.payment_prediction_model_class_weight_{{ get_suffix(ds) }}`);
"""

# 3단계: 예측 결과 저장 쿼리
PREDICT_SQL = """
CREATE OR REPLACE TABLE `codeit-final-project.json_to_table.payment_prediction_results_class_weight_{{ get_suffix(ds) }}` AS
SELECT *
FROM ML.PREDICT(MODEL `codeit-final-project.json_to_table.payment_prediction_model_class_weight_{{ get_suffix(ds) }}`,
  (
    -- 훈련 쿼리와 동일한 데이터 소스 사용
    WITH engineered_features AS (
        -- 이하 WITH 절 내용은 훈련 쿼리와 동일합니다 ...
        WITH user_events AS (
            SELECT t1.user_id, t2.event_datetime, t2.event_key, t1.session_id, t1.osname
            FROM `codeit-final-project.json_to_table.hackle_properties_{{ get_suffix(ds) }}` AS t1
            JOIN `codeit-final-project.json_to_table.hackle_events_{{ get_suffix(ds) }}` AS t2 ON t1.session_id = t2.session_id
